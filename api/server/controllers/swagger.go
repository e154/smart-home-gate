...
func (c ControllerSwagger) WrapHandler(h *webdav.Handler) gin.HandlerFunc {
	...
	swaggerYml, err := template.ParseFiles("api/server/docs/swagger.yaml")
	if err != nil {
		log.Error(err.Error())
	}
	...
	return func(ctx *gin.Context) {
		...
		case "swagger.yml":
			swaggerYml.Execute(ctx.Writer, map[string]interface{}{
				"Host": "",
			})
		default:
			...
			f, err := swaggerFiles.ReadFile(path)
			if err == nil {
				ctx.Writer.Write(f)
				return
			}
...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="./swagger-ui.css.map" >
